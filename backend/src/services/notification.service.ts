...
export class NotificationService {
    ...
    async getNotifications(userId: Notification["userId"], types: Notification["type"][], limit: number, offset: number): Promise<GetNotificationDTO[] | string> {
        ...
        const notifications = await client.query(
            "SELECT no.idnotification id, no.type, no.message, no.created_at, us.name as name FROM notifications no, contacts co  WHERE ((no.user_id = ANY(select caller from contacts c where c.called = $1) AND no.type = 3)  OR  (no.user_id = $1  AND no.type = ANY($2))) AND us.iduser = co.caller ORDER BY created_at DESC LIMIT $3  OFFSET $4",
